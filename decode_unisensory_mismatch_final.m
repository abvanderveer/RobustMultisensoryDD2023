...
limtrials1=8; %limit the number of trials analysed for each stimulus type(control/redundandt/deviant).
                ...
                if renamevecstim==1;
                    ...
                    for t=1:size(vecstim,2);
                        ...
                        elseif or(or(vecstim(1,t)==2,vecstim(1,t)==2),vecstim(1,t)==3)
                            vecstim1(1,t)=2;
                        elseif or(or(vecstim(1,t)==3,vecstim(1,t)==4),vecstim(1,t)==4)
                            vecstim1(1,t)=3;
                        elseif or(or(vecstim(1,t)==5,vecstim(1,t)==6),vecstim(1,t)==7)
                            vecstim1(1,t)=4;
                        end
                    end
                    vecstim=vecstim1;
            ...
            if limtrials1>0;
                ...
                for z1=1:4
                    ...
                    try
                        ...
                        aac=limtrials-aab;
                        effectsTR(:,:,(aab+1):limtrials,z1+1)=(tmp1(:,:,1:aac)); disp('not enough trials')
                    end
                end
                ...
                try
                    ...
                    aab=size(tmp1,3);
                    effectsTR(:,:,1:aab,6)=(tmp1(:,:,1:aab));
                    aac=limtrials-aab;
                    effectsTR(:,:,(aab+1):limtrials,6)=(tmp1(:,:,1:aac)); disp('not enough trials')
                end
                ...
                try
                    ...
                    aab=size(tmp1,3);
                    effectsTR(:,:,1:aab,1)=(tmp1(:,:,1:aab));
                    aac=limtrials-aab;
                    effectsTR(:,:,(aab+1):limtrials,1)=(tmp1(:,:,1:aac)); disp('not enough trials')
                end
...
clear effectsallTR1
%%%analyse
effectsallTR1(:,:,:,1)=mean(effectsallTR(:,:,:,1,stim),4);effectsallTR1(:,:,:,2)=mean(effectsallTR(:,:,:,6,stim),4);

 %
time1=BEGINo; time2=ENDo;%time2=BEGINo+3;%
trs=size(effectsallTR1,3);
sampa=4; %how many you use to create the average
...
aa=((mean(devvec,2)+mean(typvec,2))/2);%-((mean(ranvec,2)+mean(ranvec,2))/2)+.5;
mouseavs(file)=mean(aa);%((BEGINo+3):ENDo));
mouseavsTIME(file,:)=aa';

nummacells(file)=size(c,1)./((time2-time1)+1);

aa=((mean(ranvec,2)+mean(ranvec,2))/2);
mouseavs_ran(file)=mean(aa);%((BEGINo+3):ENDo));
end
...
figure;errorbar_groups([mean(mouseavs-mouseavs_ran)+.5]',[std(mouseavs-mouseavs_ran)./sqrt(file)]', 'bar_colors',[.5 .5 .5]);hold on; scatter((mouseavs-mouseavs)+1,(mouseavs-mouseavs_ran)+.5,200,'k'); ylim([0 1]); make_eps_saveable
